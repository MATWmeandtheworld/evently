...
// Define the VenuePerformance type based on usage in the code
...
// Define the RevenueByCategory type based on usage in the code
...
export function AdminAnalyticsComponent() {
  ...
  data={analytics.venuePerformance.slice(0, 5).map((venue: { venue: string; revenue: number }) => ({
...
/>
  ...
  <SimpleChart
    ...
    data={analytics.eventTrends.map((trend: { month: string; events: number }) => ({
      label: trend.month,
      value: trend.events,
    }))}
    type="line"
  ...
  <SimpleChart
    ...
    data={analytics.userGrowth.map((growth: { month: string; admins: number; organizers: number; attendees: number }) => ({
      label: growth.month,
  ...
  <SimpleChart
    ...
    data={analytics.revenueByCategory.map((category: { category: string; revenue: number }) => ({
  ...
  />
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {analytics.venuePerformance.slice(0, 4).map(
                    (venue: VenuePerformance, index: number) => (
        ...
        <TabsContent value="trends" className="space-y-6">
            ...
            <SimpleChart
              title="User Growth"
              description="Platform user acquisition over time"
              data={analytics.userGrowth.map(
              (growth: { month: string; admins: number; organizers: number; attendees: number }) => ({
                label: growth.month,
                value: growth.admins + growth.organizers + growth.attendees,
              })
              )}
        ...
        <TabsContent value="revenue" className="space-y-6">
            ...
            <SimpleChart
              title="Revenue by Category"
              description="Revenue distribution across event categories"
              data={analytics.revenueByCategory.map((category: RevenueByCategory) => ({
              label: category.category,
              value: category.revenue,
              }))}
              type="pie"
            ...
            <Card>
              ...
              <CardContent>
              <div className="space-y-4">
                {analytics.revenueByCategory.map((category: RevenueByCategory, index: number) => (
                <div key={index} className="flex items-center justify-between">
